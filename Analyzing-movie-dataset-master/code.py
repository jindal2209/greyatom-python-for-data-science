...
def rate_bucket(dataset, rate_low, rate_high):
    rated_movies = []
    for row in dataset:
        if float(row[11]) >= rate_low and float(row[11]) <= rate_high :
...
for row in movies:
    if int(row[12]) > max_review:
        max_review = int(row[12])
        
for row in movies:
    if int(row[12]) == max_review:
        reviews_max[row[13]] = row[12]
...
for row in movies:
    name = row[13]
    rev_count = int(row[12])
    for row1 in movies_clean:
        if row1[13] == name and int(row1[12]) < rev_count :
            movies_clean.remove(row1)
...
high_rated_movies = rate_bucket(movies_en,8.0,10)
